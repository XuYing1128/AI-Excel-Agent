...
- **常见任务有专用技能流程。** 模板填充、排考、绩效薪酬、数据清洗、报表图表等任务会自动注入对应 skill，优先调用现有稳定工具。
...
3. 首次运行时安装依赖。
4. 启动前检查服务状态。
5. 打开 `http://127.0.0.1:8501`。

如果你想提前安装测试依赖，再双击 `install.bat`。
...
- 审查结果：用于生成后的主观建议审查。
- 分类、摘要、起名：预留给便宜快速模型。
- 编写安全脚本：用于受限 `run_python` 工具处理长尾数据任务。

同时可以控制：

- 是否启用自动多步生成。
- 是否允许安全脚本工具。开启后，模型可让本地子进程在当前任务目录内执行受限 Python：限时运行、只读 `input/`、只写 `output/` 和临时目录、禁网络，并检查用户代码 import 白名单；介意时可关闭。
